...
$PhoenixReleaseVersion = "v2.0_260706"
$PhoenixSupportedUpgradeFrom = @("v1.6", "v1.7", "v1.7_260619", "v1.7_260626", "v1.8", "v1.8_260625", "v1.9", "v1.9_260702")
...
Write-Host "This updater preserves existing bots, refreshes the proactive runner, preserves v2.0 PCS/PTS, and upgrades prior installs to Phoenix Agent v2.0 Web Control Agent. v1.5 users should use cleaner then installer."
...
$repairSrc = Join-Path $PSScriptRoot "phoenix_v20_auth_order_repair.cjs"
...
function Test-PhoenixInstalledTarget {
  ...
  if ($found.Count -eq 0) {
    throw "No installed Phoenix bot work folder/profile was found for target '$BotName'. If the previous install failed or you are in the wrong Windows user account, use the v2.0 installer instead of the updater. v1.5 users should clean reinstall with the v2.0 cleaner and installer."
  }
  return $found
}
...
function Write-PhoenixReleaseMarker {
  ...
  $marker = [ordered]@{
    ...
    updaterRole = "preserve existing installs and upgrade prior Phoenix Agent installs to v2.0_260706 Web Control Agent with v2.0 PCS/PTS preserved"
  }
...
function Test-Phoenixv19FeatureUpgrade {
  ...
  Write-Host ""
  Write-Host "[4/5] v2.0 PCS/PTS feature upgrade audit" -ForegroundColor Cyan
  ...
  if (-not (Test-Path -LiteralPath $dest -PathType Leaf)) {
    ...
    if ($runnerMissing.Count -gt 0) {
      Write-Host "  WARN runner: missing v2.0/compat marker(s): $($runnerMissing -join ', ')" -ForegroundColor Yellow
      $allOk = $false
    } else {
  ...
  foreach ($bot in $Bots) {
    ...
    if ($missingFiles.Count -gt 0 -or $missingMarkers.Count -gt 0) {
      ...
      if ($missingMarkers.Count -gt 0) {
        Write-Host "  WARN ${bot}: missing v2.0/compat marker(s): $($missingMarkers -join ', ')" -ForegroundColor Yellow
      }
      $allOk = $false
    } else {
      Write-Host "  OK ${bot}: v2.0 identity, proactive policy, PCS/PTS folders, web-control folders, and skill-up markers applied"
    }
  }

  if ($allOk) {
    Write-Host "  v2.0 PCS/PTS feature upgrade audit: OK" -ForegroundColor Green
  } else {
    Write-Host "  v2.0 PCS/PTS feature upgrade audit: review warnings above" -ForegroundColor Yellow
  }
  return $allOk
}
...
if (-not (Test-Path -LiteralPath $root)) {
  throw "OpenClaw workspace not found: $root. If the previous install failed or was deleted, run the v2.0 installer. If you are using another Windows account, switch to the account where the bots were installed."
}
...
if ($openaiRepairBots.Count -gt 0) {
  ...
  Write-Host "  Codex provider plugin check is handled by phoenix_v20_auth_order_repair.cjs to avoid duplicate OpenClaw plugin installs." -ForegroundColor DarkGray
...
Write-Host ""
Write-Host "[2/5] Upgrading bots to Phoenix Agent v2.0 web control features while preserving v2.0 PCS/PTS" -ForegroundColor Cyan
...
foreach ($targetBot in $targetBots) {
  ...
  if (Test-Path -LiteralPath $workDir -PathType Container) {
    ...
    Install-Phoenixv20WebControlStructure -RootDir $root -WorkDir $workDir -BotName $targetBot -DisplayName $display -BotId $knownBotProcesses[$targetBot]
  }
}
...
if ($botProcesses.Count -gt 0) {
  ...
  Write-Host "If this bot should be installed, run the v2.0 installer or check that you are in the same Windows user account where the previous Phoenix Agent was installed." -ForegroundColor Yellow
}
Write-Host ""
