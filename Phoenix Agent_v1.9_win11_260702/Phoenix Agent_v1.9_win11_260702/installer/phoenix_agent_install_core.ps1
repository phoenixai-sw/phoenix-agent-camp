...
function Invoke-PhoenixAuthOrderRepair {
  ...
  $src = Join-Path $PSScriptRoot 'phoenix_v19_auth_order_repair.cjs'
  if (-not (Test-Path -LiteralPath $src)) {
    Write-Host "  WARNING: auth order repair payload missing; skipping v1.9 auth order repair." -ForegroundColor Yellow
    return
  }
  if (-not (Get-Command node -ErrorAction SilentlyContinue)) {
    Write-Host "  WARNING: node command not found; skipping v1.9 auth order repair." -ForegroundColor Yellow
    return
  }
  & node $src --bot $BotName
  if ($LASTEXITCODE -ne 0) {
    Write-Host "  WARNING: v1.9 auth order repair could not find a valid Codex OAuth profile yet." -ForegroundColor Yellow
  }
}
...
Write-Host "Phoenix Agent v1.9 Win11 installer: $displayName" -ForegroundColor Cyan
...
$phoenixRoleDetail = switch ($botName) {
  ...
  default { 'Phoenix Agent v1.9 role-specific operation.' }
}
...
You are $displayName, a Phoenix Agent v1.9 Telegram/OpenClaw bot.
PM2 profile: $botId
