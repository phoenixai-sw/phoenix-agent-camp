...
function esc(s) { return s.replace(/[.*+?^${}()|[\]\\]/g, '\\configure_proactive_heartbeat() {'); }
...
repair_auth_order_v17() {
  local bot_name="$1"
  local src="$SCRIPT_DIR/phoenix_v19_auth_order_repair.cjs"
  if [[ ! -f "$src" ]]; then
    echo "  WARNING: auth order repair payload missing; skipping v1.9 auth order repair." >&2
    return 0
  fi
  if ! command -v node >/dev/null 2>&1; then
    echo "  WARNING: node command not found; skipping v1.9 auth order repair." >&2
    return 0
  fi
  if ! node "$src" --bot "$bot_name"; then
    echo "  WARNING: v1.9 auth order repair could not find a valid Codex ChatGPT login profile yet." >&2
  fi
}
...
echo "Phoenix Agent v1.9 macOS installer: $DISPLAY_NAME"
...
EOF_DETAIL
  ...
  writer)
    ...
    ;;
  *)
    PHOENIX_ROLE_DETAIL="Korean role markers: Phoenix Agent v1.9 role-specific operation."
    ;;
esac
...
You are __DISPLAY_NAME__, a Phoenix Agent v1.9 Telegram/OpenClaw bot.
