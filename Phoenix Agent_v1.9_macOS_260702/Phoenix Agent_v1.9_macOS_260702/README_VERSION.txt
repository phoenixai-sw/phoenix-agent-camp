...
- configure_laptop_awake_mode.command is an optional macOS helper. It checks pmset status or applies power-adapter awake settings only when the user runs it.
- Laptop lid operation is an OS/hardware setting, not a pure Phoenix Agent software feature.
- Local bots keep running only while the Mac is powered on, awake, connected to the internet, and PM2/OpenClaw gateway processes are alive.
