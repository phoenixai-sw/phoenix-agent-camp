...
function migrateCodexToOpenAI(pm2Name) {
  // Some OpenClaw versions can leave migrate children waiting. v1.9 repair
  // performs the same intended binding directly: copy Codex auth into the bot
  // CODEX_HOME and pin the OpenAI provider to the codex-imported OAuth profile.
  return false;
}
