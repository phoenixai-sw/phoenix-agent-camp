...
PHOENIX_RELEASE_VERSION="v1.9_260702"
PHOENIX_SUPPORTED_UPGRADE_FROM="v1.6,v1.7,v1.7_260619,v1.7_260626,v1.8,v1.8_260625"
SRC="$SCRIPT_DIR/phoenix_proactive_nudge.cjs"
DEST="$ROOT/Phoenix_Proactive_Nudge.cjs"
REPAIR_SRC="$SCRIPT_DIR/phoenix_v19_auth_order_repair.cjs"
...
echo "This updater preserves existing bots, refreshes the proactive runner, and upgrades v1.6/v1.7/v1.8 installs to Phoenix Agent v1.9 with PCS/PTS skill-up structure. v1.5 users should use cleaner then installer."
...
function esc(s) { return s.replace(/[.*+?^${}()|[\]\\]/g, '\\test_installed_target() {'); }
...
const common = path.join(root, 'phoenix_v19');
...
console.log('  OK ' + display + ': Phoenix Agent v1.9 PCS/PTS skill-up structure installed.');
NODE
}
test_installed_target() {
  ...
  if [[ "${#INSTALLED_TARGET_BOTS[@]}" -eq 0 ]]; then
    ...
    echo "If the previous install failed or you are in the wrong macOS user account, use the v1.9 installer instead of the updater. v1.5 users should clean reinstall with the v1.9 cleaner and installer." >&2
    exit 1
  fi
}
...
audit_v19_feature_upgrade() {
  ...
  echo
  echo "[4/5] v1.9 PCS/PTS feature upgrade audit"
  ...
  if [[ "$all_ok" -eq 0 ]]; then
    echo "  v1.9 PCS/PTS feature upgrade audit: OK"
  else
    echo "  v1.9 PCS/PTS feature upgrade audit: review warnings above" >&2
  fi
  return "$all_ok"
}
...
if [[ ! -d "$ROOT" ]]; then
  ...
  echo "If the previous install failed or was deleted, run the v1.9 installer. If you are using another macOS account, switch to the account where the bots were installed." >&2
  exit 1
fi
if [[ ! -f "$SRC" ]]; then
...
if [[ "${#OPENAI_REPAIR_BOTS[@]}" -gt 0 ]]; then
  ensure_codex_cli_first_auth
  echo "  Codex provider plugin check is handled by phoenix_v19_auth_order_repair.cjs to avoid duplicate OpenClaw plugin installs."
...
echo
echo "[2/5] Upgrading bots to Phoenix Agent v1.9 proactive + PCS/PTS features"
...
if [[ -n "$RUNNING_BOTS" ]]; then
  ...
  echo "If this bot should be installed, run the v1.9 installer or check that you are in the same macOS user account where the previous Phoenix Agent was installed."
fi
echo
echo "[5/5] PM2 status"
pm2 list
